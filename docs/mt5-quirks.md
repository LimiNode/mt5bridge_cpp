...
| `trade-reconciliation` | After a trading call, orders/deals/positions may become mutually consistent only after a delay; a timeout may arrive after server acceptance | Persist a durable dispatch intent before `order_send`, never blindly retry, and use a bounded ticket/identifier-graph confirmation phase with explicit `CONFIRMED`/`PENDING`/`NOT_OBSERVED`/`AMBIGUOUS` states; `NOT_OBSERVED` never proves absence or permits retry | [MT4Orders discussion](https://www.mql5.com/zh/forum/164943/page64), [order_send](https://www.mql5.com/en/docs/python_metatrader5/mt5ordersend_py) |
