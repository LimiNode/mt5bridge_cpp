...
| `trade-reconciliation` | After a trading call, orders/deals/positions may become mutually consistent only after a delay; a timeout may arrive after server acceptance | Never blindly retry `order_send`; expose the raw result and use a separate bounded, ticket-first confirmation phase with explicit `CONFIRMED`/`PENDING`/`NOT_FOUND`/`AMBIGUOUS` states | [MT4Orders discussion](https://www.mql5.com/zh/forum/164943/page64), [order_send](https://www.mql5.com/en/docs/python_metatrader5/mt5ordersend_py) |
