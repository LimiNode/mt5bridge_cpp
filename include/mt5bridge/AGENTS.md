...
ABI, `client.hpp` is the lightweight C++ consumer facade, and the domain
umbrellas are `market.hpp`, `trade.hpp`, `observation.hpp`, and `dispatch.hpp`.
Focused contracts live under `market/`, `trade/`, `observation/`,
`reconciliation/`, and `dispatch/`; the former flat paths remain forwarding
headers for source compatibility. All focused headers remain self-contained
and none pulls in the heavy runtime.
...
- `market/data.h` contains only POD requests/results, opaque buffer handles,
  and C callbacks. Keep it free of STL, exceptions, and Python/JSON
  dependencies.
- `trade/observation.h` contains the typed Stage 1 account/symbol/order-check
  PODs. Use its
